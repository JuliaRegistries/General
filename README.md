...
1. Follow Github's documentation on [splitting a subfolder out into a new repository][github-subfolder] an set the subdirectory package to be the new root folder of the new repository (most of these steps are still applicable for packages not hosted on GitHub). In order to avoid having to modify the package's `git-tree-sha1` entries in the registry it is important that subdirectory package's contents remain identical for the registered releases. This means one should not use `git-filter-repo`'s `--path` functionality to transfer additional top-level directories (like `.github/workflows`) when transferring the history, as this would modify root tree SHA.
